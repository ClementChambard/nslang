...
Clap Clap::parse(int argc, char **argv) {
  ...
  while (argc > 0) {
    ...
    } else if (std::strcmp(*argv, "-O0") == 0) {
      out.opt_level = 0;
    } else if (std::strcmp(*argv, "-O1") == 0) {
      out.opt_level = 1;
    } else if (std::strcmp(*argv, "-O2") == 0) {
      out.opt_level = 2;
    } else if (std::strcmp(*argv, "-O3") == 0) {
      out.opt_level = 3;
    } else {
...
Mode get_mode(Clap const &c) {
  ...
  if (c.print_ast)
    mode = Mode::PRINT_AST, set++;
  if (c.print_llvm)
    mode = Mode::PRINT_LLVM, set++;
  if (c.print_asm)
    mode = Mode::PRINT_ASM, set++;
  if (c.compile_only)
    mode = Mode::COMPILE_ONLY, set++;

  if (set > 1) {
    fatal_error(
        "only one of [-c, -asm, -ast, -llvm] can be set at a time");
  }
...
CheckedClap Clap::check() const {
  ...
  out.opts.include_path.insert(out.opts.include_path.end(),
                               included_dirs.begin(), included_dirs.end());
  out.opts.link_libraries.insert(out.opts.link_libraries.end(),
                                 linked_libs.begin(), linked_libs.end());

  if (supplied_filenames.size() == 0)
    fatal_error("no input file");

  if (supplied_filenames.size() > 1 &&
      out.opts.mode != Mode::COMPILE_AND_LINK && output_filename) {
    fatal_error("cannot supply output filename with multiple files");
  }

  if (out.opts.mode != Mode::COMPILE_AND_LINK &&
      out.opts.mode != Mode::COMPILE_ONLY && output_filename) {
    fatal_error("cannot supply output filename with one of [-asm, -ast, -llvm, "
                "-llvm-opt]");
  }

  if (out.opts.mode == Mode::COMPILE_ONLY && output_filename) {
    assert(supplied_filenames.size() == 1);
    if (is_linktime_file(supplied_filenames[0]))
      fatal_error("file " + supplied_filenames[0] + " is already compiled");
    out.files_to_compile.emplace_back(supplied_filenames[0], *output_filename);
  } else {
    for (auto &f : supplied_filenames) {
      if (is_linktime_file(f)) {
        if (out.opts.mode != Mode::COMPILE_AND_LINK)
          fatal_error("file " + f + " is already compiled");
        ...
        if (out.opts.mode != Mode::COMPILE_ONLY)
          out.files_to_remove.push_back(out_fn);
      }
    }
  }
  ...
  if (out.opts.mode == Mode::COMPILE_AND_LINK) {
    ...
    if (output_filename)
      out.program_name = *output_filename;
    else
      out.program_name = "a.out";
  }
