...
class Scope:
    ...
    def set_flags_pr(self, parent: Self | None, f: int):
        ...
        if parent is not None:
            ...
            if (f & (ScopeFlags.FN | ScopeFlags.CLASS | ScopeFlags.BLOCK | ScopeFlags.FUNCTION_PROTO)) == 0:
    ...
    def is_switch_scope(self) -> bool:
        ...
        while s is not None:
            if (s.flags & ScopeFlags.SWITCH) != 0:
                return True
            elif (s.flags & (ScopeFlags.FN | ScopeFlags.CLASS | ScopeFlags.BLOCK | ScopeFlags.FUNCTION_PROTO)) != 0:
                return False
            s = s.get_parent()
        return False

    def is_loop_scope(self) -> bool:
        return (self.flags & ScopeFlags.BREAK) != 0 and (self.flags & ScopeFlags.SWITCH) == 0
    ...
    def set_is_condition_var_scope(self, val: bool):
        self.flags = (self.flags & ~ScopeFlags.CONDITION_VAR) | (int(val) * ScopeFlags.CONDITION_VAR )

    def get_next_function_prototype_index(self) -> int:
        assert self.is_function_prototype_scope()
    ...
    def lookup_named_decl(self, decl_name) -> Decl | None:
