...
class StringLiteralParser:
    ...
    def parse(self, string_toks: List[Token]):
        if len(string_toks) == 0 or len(string_toks[0].value) < 2:
            return self.diagnose_lexing_error(Loc())

        self.max_token_length = len(string_toks[0].value)
        self.size_bound = len(string_toks[0].value) - 2
        ...
        for tok in string_toks:
            ...
            self.size_bound += len(tok.value) - 2;

            if len(tok.value) > self.max_token_length:
                self.max_token_length = len(tok.value)
            ...
            if not self.eval_method and tok.ty != Tok.STR:
                ...
                diag(tok.loc, "encoding prefix '{prefix}' on an unevaluated string literal has no effect" , Diag.ERROR) # << Prefix << FixItHint::CreateRemoval(Range);
                had_error = True
            # else change kind for concat

        self.size_bound += 1
        self.char_byte_width = get_char_width(self.kind)
        assert (self.char_byte_width & 7) == 0, "Assumes character size is byte multiple"
        ...
        for i in range(len(string_toks)):
            this_token_val = string_toks[i].value
            ...
            while len(this_token_val) > 0:
                if this_token_val[0] != "\\":
                    ...
                    while count < len(this_token_val) and this_token_val[count] != "\\":
                        count += 1
                    if self.copy_string_fragment(string_toks[i], this_token_val[:count]):
                        self.had_error = True
                    this_token_val = this_token_val[count:]
                    continue
                if this_token_val[1] == "u" or this_token_val[1] == "U" or this_token_val[1] == "N":
                    self.encode_ucn_escape(asdjaaskldjldsjdlskaj)
                    continue
                result_char, to_skip = self.process_char_escape(this_token_val, string_toks[i].loc)
    ...
    def process_char_escape(self, buf: str, loc: Loc):
        cur_loc = 0
